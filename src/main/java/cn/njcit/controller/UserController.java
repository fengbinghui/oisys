...
import cn.njcit.constant.SystemConstants;
...
public class UserController {
    ...
    public ResponseResult saveStudent(User user, @RequestParam(required = false) MultipartFile filePath,
        ...
        if (user.getUserId() == null) {// 用户对象的用户id是null，表示新增
            ...
            wrapper.eq("is_lock", 0).eq("role_id", SystemConstants.STUDENT_ROLE_ID).eq("user_name", user.getUserName());
            ...
            user.setRoleId(SystemConstants.STUDENT_ROLE_ID);// 设置role_id是4
        ...
        if (!StringUtil.isEmpty(imgpath)) {// 如果头像上传成功，上传后的文件名不能为空
            ...
            user.setImgPath(SystemConstants.DEFAULT_STUDENT_AVATAR);// 为学生设置一个固定的头像
        }// 注意：如果是修改信息，且没有重新上传头像，维持user对象里的imgPath不变，即不跟该本来的头像
        if (resetPassword) {
            user.setPassword(SystemConstants.DEFAULT_STUDENT_PASSWORD);// 如果是用户选择重置密码，或重置密码（默认密码123456）
        }

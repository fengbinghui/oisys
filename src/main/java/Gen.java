...
public class Gen {
    public static void main(String[] args) {
        String url = "jdbc:mysql://localhost:3308/oisys-ren?useSSL=false&useUnicode=true&characterEncoding=UTF-8&serverTimezone=GMT%2B8";
        FastAutoGenerator.create(url, "root", "123456").globalConfig(builder -> {
                    builder.author("selena") // 设置作者
                            .outputDir("F://output"); // 指定输出目录
                }).packageConfig(builder -> {
                    builder.parent("cn.njcit") // 设置父包名
                            .pathInfo(Collections.singletonMap(OutputFile.xml, "F://output/mapper")); // 设置mapperXml生成路径
